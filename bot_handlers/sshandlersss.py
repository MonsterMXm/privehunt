...
auto_strategies = AutoStrategies()
...
async def cmd_start(message: types.Message):
    ...
    try:
        ...
        user_data = await db.fetch(
            "SELECT free_signals, vip_until, auto_trading FROM users WHERE user_id = ?",
            (message.from_user.id,))
        
        free_signals = user_data[0][0] if user_data and user_data[0][0] is not None else Config.FREE_SIGNALS
        vip_status = "✅ Активна" if user_data and user_data[0][1] and datetime.now() < datetime.fromisoformat(user_data[0][1]) else "❌ Неактивна"
        auto_trading_status = "🟢 ВКЛ" if user_data and user_data[0][2] else "🔴 ВЫКЛ"
        
        # Отправка приветственного сообщения
        await message.answer(
            f"🚀 Добро пожаловать, {message.from_user.full_name}!\n\n"
            f"🔹 Ваш статус: {'VIP' if vip_status == '✅ Активна' else 'Обычный'}\n"
            f"🔹 Автоторговля: {auto_trading_status}\n"
            f"🔹 Доступно сигналов: {free_signals}\n\n"
...
async def cmd_help(message: types.Message):
    ...
    help_text = (
        ...
        "Автоторговля:\n"
        "/autotrade - Управление автоторговлей\n"
...
async def cmd_ai_scan(message: types.Message):
    ...
    try:
        ...
        await message.answer("ℹ️ Используйте: /ai_scan BTC/USDT")
    except Exception as e:
